...
import { Avatar, Card, Text, Button, Modal, Portal, TextInput, Divider, IconButton,Provider } from 'react-native-paper';
...
const initialFeedback = [
    { id: 'f1', municipality: 'Kamalamai Municipality', rating: 5, comment: 'Excellent waste collection service this month.', date: '2025-09-01', department: 'Administration' },
    { id: 'f2', municipality: 'Dudhauli Municipality', rating: 4, comment: 'Pothole repair was quick and effective.', date: '2025-09-15', department: 'Engineering' },
];

const initialGrievance = [
    { id: 'g1', title: 'Street Light Outage', description: 'Street light broken on main road near Ward 2 office.', status: 'In Progress', date: '2025-10-01' },
];
...
const FeedbackAndGrivanceScreen = ({ navigation }) => {
    ...
    // Municipality Selector Modal State
    const [isMunicipalitySelectorVisible, setIsMunicipalitySelectorVisible] = useState(false);
    const [isDepartmentSelectorVisible, setIsDepartmentSelectorVisible] = useState(false);


    // Effect to update selected municipality when the list changes (e.g., when navigating from a new district)
    useEffect(() => {
    ...
    const hideFeedbackModal = () => setIsFeedbackVisible(false);
    
    const showGrievanceModal = () => setIsGrievanceVisible(true);
    const hideGrievanceModal = () => setIsGrievanceVisible(false);

    const submitFeedback = () => {
        if (feedbackRating === 0 || !feedbackComment || !selectedMunicipality) {
            console.error("Please provide a rating, select a municipality, and enter a comment.");
            return;
        }
    ...
    const renderGrievanceItem = ({ item }) => (
        <Card mode="outlined" style={styles.listItem}>
            <Card.Content>
                ...
                <Text style={styles.itemDate}>Status: <Text style={{ color: item.status === 'Resolved' ? 'green' : (item.status === 'Open' ? 'red' : 'orange') }}>{item.status}</Text></Text>
    ...
    return (
        <SafeAreaProvider>
          <Provider>
            ...
            {/* --- Municipality Selector Modal (Re-usable for Feedback) --- */}
            <Portal>
                ...
                <Modal visible={isGrievanceVisible} onDismiss={hideGrievanceModal} contentContainerStyle={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        ...
                        <TextInput
                            ...
                            mode="outlined"
                            style={styles.fixedInput}
                            right={<TextInput.Icon icon="menu-down" onPress={() => setIsDepartmentSelectorVisible(true)} />}
                            editable={false}
                        />
            ...
            {/* --- Department Selector Modal (Re-usable for Grievance) --- */}
            <Portal>
                <Modal visible={isDepartmentSelectorVisible} onDismiss={() => setIsDepartmentSelectorVisible(false)} contentContainerStyle={styles.modalContainer}>
                    <Text variant="titleMedium" style={styles.modalTitle}>Select Department</Text>
                    {renderSelectorList(
                        DEPARTMENTS, 
                        grievanceDepartment, 
                        setGrievanceDepartment, 
                        () => setIsDepartmentSelectorVisible(false)
                    )}
                </Modal>
            </Portal>
            </Provider>
